...
#### 7. Nomination Request (Customer Portal) — **BARU**

**Definisi**  
Modul yang menyediakan portal bagi customer (Pemilik Kapal/Shipper) untuk mengajukan permintaan nominasi kapal sebelum memasuki area perairan STS Bunati. Data nominasi yang disubmit melalui LPS Portal akan diteruskan ke STS Platform untuk proses validasi, approval, resource assignment, dan scheduling.

**Tujuan**

- Menyediakan single entry point bagi customer untuk mengajukan nominasi kapal melalui LPS Platform.
- Memfasilitasi proses registrasi dan pengajuan nominasi secara digital.
- Meneruskan data nominasi ke STS Platform untuk proses approval dan operasional selanjutnya.
- Menyediakan visibilitas status nominasi kepada customer secara real-time.

**Cakupan**

_Registrasi & Akun Customer_

- Registrasi akun customer baru melalui LPS Portal (self-registration).
- Validasi data registrasi oleh Admin sebelum akun aktif.
...
_Tracking & Monitoring_

- Customer dapat melihat status nominasi secara real-time dengan lifecycle lengkap: **Draft** → **Submitted** → **Pending** (Menunggu proses di STS Platform) → **Approved** / **Need Revision** → **Waiting Payment Verification** → **Payment Confirmed** / **Payment Rejected**.
- Customer menerima notifikasi saat status nominasi berubah.
- Customer dapat melihat EPB (Estimasi Perkiraan Biaya) yang digenerate oleh STS Platform beserta nominal yang harus dibayarkan.
- Customer dapat melihat detail jadwal (schedule, slot) setelah nominasi di-approve.

_EPB Confirmation & Payment Proof Upload_

- Setelah nominasi di-approve oleh STS Platform dan EPB dikirimkan ke LPS, customer akan melihat detail EPB beserta nominal yang harus dibayarkan.
- Customer harus mengupload bukti pembayaran (Proof of Payment) melalui LPS Portal sesuai dengan EPB yang diterima.
- Format file yang didukung: PDF, JPG, PNG (max 5MB per file).
- Setelah customer mengupload bukti pembayaran, data bukti bayar dikirimkan ke STS Platform via API.
- Status nominasi berubah menjadi "Waiting Payment Verification" setelah upload bukti bayar.
- STS Platform melakukan verifikasi pembayaran. Hasil verifikasi (Confirmed/Rejected) dikirim kembali ke LPS dan ditampilkan ke customer.
- Jika pembayaran dikonfirmasi oleh STS, status berubah menjadi "Payment Confirmed" dan voyage dapat dimulai.
- Jika pembayaran ditolak oleh STS, customer menerima notifikasi beserta alasan dan dapat mengupload ulang bukti pembayaran yang benar.

_Integrasi dengan STS Platform_

- Data nominasi dikirim ke STS Platform via API setelah customer melakukan submit.
- Status update dari STS Platform (approval, revision, EPB) diterima oleh LPS dan ditampilkan ke customer.
- Bukti pembayaran yang diupload customer dikirimkan ke STS Platform via API untuk verifikasi.
- Hasil verifikasi pembayaran dari STS Platform diterima oleh LPS dan ditampilkan ke customer.
- LPS tidak melakukan proses approval, resource assignment, scheduling, maupun verifikasi pembayaran — seluruhnya dilakukan oleh STS Platform.

_Track Voyage (Post-Payment Confirmed)_

- Customer dapat memantau status voyage yang sedang berjalan (view-only) termasuk posisi kapal dari data AIS.

_Customer Portal Dashboard_

- Customer landing dashboard menampilkan ringkasan: daftar nominasi aktif beserta status terkini, daftar voyage yang sedang berjalan, dan widget cuaca real-time area STS Bunati.
- Dashboard customer bersifat view-only; data bersumber dari LPS (cuaca, AIS) dan STS Platform (status nominasi, voyage).
- Customer dapat mengakses data cuaca real-time dan history alert area perairan STS Bunati melalui menu Weather & Alert di Customer Portal (view-only).

> **Batasan Modul:** Modul Nomination di LPS bersifat **portal submit, upload bukti bayar, dan monitoring** saja. Proses bisnis approval, pengecekan ketersediaan resource, penjadwalan, assignment anchor/crane/barge, generate EPB, dan verifikasi pembayaran seluruhnya dikelola oleh STS Platform.

#### 8. Monitoring & Visibility Dashboard — **BARU (Pengganti Reporting & Analytics)**
...
#### 9. System Configuration (Sub-modul dari Master Data lama)
...
| M7  | Nomination Request (Customer Portal) | Customer Portal       |
| M8  | Monitoring & Visibility Dashboard    | Monitoring            |
| M9  | System Configuration                 | Konfigurasi           |
...
| Nomination Request     |      —       |      —       |    —     | Approve Reg. |  —   |     —     | Submit, Upload Payment & Track |    View    |
...
#### 3.4.7. Nomination Request (Customer Portal) — **BARU**

| FR ID    | Requirements                                                                                                                                                                                                                                           | Actor    |
| -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------- |
| FR-NR-01 | Sistem harus menyediakan halaman registrasi bagi customer baru dengan field: Company Name, NPWP, Contact Person, Email, Phone                                                                                                                          | Customer |
| FR-NR-02 | Admin harus dapat memvalidasi dan mengaktifkan akun customer yang baru mendaftar                                                                                                                                                                       | Admin    |
| FR-NR-03 | Customer yang sudah terverifikasi harus dapat login dan mengakses dashboard customer                                                                                                                                                                   | Customer |
| FR-NR-04 | Sistem harus menyediakan form pengajuan nominasi dengan field: Vessel Name, ETA, Cargo Type, Cargo Quantity, Charterer, Estimasi Jumlah Barge                                                                                                          | Customer |
| FR-NR-05 | Customer harus dapat mengupload dokumen pendukung nominasi: Rencana Kerja, Shipping Instruction, Surat Penunjukan PBM, Nomor PKK (input manual)                                                                                                        | Customer |
| FR-NR-06 | Customer harus dapat menyimpan nominasi sebagai Draft sebelum melakukan Submit                                                                                                                                                                         | Customer |
| FR-NR-07 | Sistem harus mengenerate Nomor Nominasi otomatis setelah customer melakukan Submit                                                                                                                                                                     | System   |
| FR-NR-08 | Sistem harus mengirimkan data nominasi ke STS Platform via API setelah customer melakukan Submit                                                                                                                                                       | System   |
| FR-NR-09 | Sistem harus menerima status update nominasi dari STS Platform (Pending, Approved, Need Revision) dan menampilkannya ke customer. Status Pending ditampilkan dengan label "Menunggu proses di STS Platform".                                           | System   |
| FR-NR-10 | Customer harus dapat melihat EPB (Estimasi Perkiraan Biaya) yang digenerate oleh STS Platform beserta detail nominal yang harus dibayarkan                                                                                                             | Customer |
| FR-NR-11 | Customer harus dapat melihat detail jadwal (anchor point, ETB, estimasi durasi) setelah nominasi di-approve oleh STS Platform                                                                                                                          | Customer |
| FR-NR-12 | Sistem harus mengirim notifikasi ke customer saat status nominasi berubah                                                                                                                                                                              | System   |
| FR-NR-13 | Customer harus dapat melakukan revisi data nominasi jika status = Need Revision, kemudian re-submit                                                                                                                                                    | Customer |
| FR-NR-14 | Customer harus dapat mengupload bukti pembayaran (Proof of Payment) melalui LPS Portal sesuai dengan EPB yang diterima dari STS Platform. Format yang didukung: PDF, JPG, PNG (max 5MB)                                                                | Customer |
| FR-NR-15 | Sistem harus mengirimkan data bukti pembayaran yang diupload customer ke STS Platform via API untuk proses verifikasi                                                                                                                                  | System   |
| FR-NR-16 | Setelah customer mengupload bukti pembayaran, status nominasi harus berubah menjadi "Waiting Payment Verification"                                                                                                                                     | System   |
| FR-NR-17 | Sistem harus menerima hasil verifikasi pembayaran dari STS Platform (Confirmed/Rejected) dan menampilkannya ke customer                                                                                                                                | System   |
| FR-NR-18 | Jika pembayaran ditolak oleh STS Platform, customer harus menerima notifikasi beserta alasan penolakan dan dapat mengupload ulang bukti pembayaran yang benar                                                                                          | Customer |
| FR-NR-19 | Setelah pembayaran dikonfirmasi oleh STS Platform, status nominasi berubah menjadi "Payment Confirmed" dan voyage dapat dimulai                                                                                                                        | System   |
| FR-NR-20 | Customer harus dapat memantau status voyage yang sedang berjalan (view-only) termasuk posisi kapal dari data AIS                                                                                                                                       | Customer |
| FR-NR-21 | Sistem harus menampilkan halaman Nomination Status dengan daftar seluruh nominasi customer beserta status terkini, termasuk nominasi berstatus Draft                                                                                                   | Customer |
| FR-NR-22 | Sistem harus menampilkan Customer Portal Dashboard setelah customer login, berisi: ringkasan nominasi aktif beserta status terkini, daftar voyage yang sedang berjalan (view-only, data dari STS Platform), dan widget cuaca real-time area STS Bunati | Customer |
| FR-NR-23 | Customer harus dapat melihat data cuaca real-time dan history alert area perairan STS Bunati melalui Customer Portal (view-only)                                                                                                                       | Customer |

#### 3.4.8. Monitoring & Visibility Dashboard — **BARU**
